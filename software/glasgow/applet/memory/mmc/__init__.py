...
class MemoryMmcApplet(GlasgowApplet):
    ...
    async def interact(self, device, args, iface: MmcInterface):
        ...
        if args.operation == "info":
            ...
            log.info("- Card Capacity Status: %s", iface.ccs)
            log.info("- UHS-II Card: %s", iface.uhsii)
            log.info("- 1.8v Switching Accepted: %s", iface.s18a)
            log.info("- 2.7v-2.8v: %s", iface.ocr.v27_28)
            log.info("- 2.8v-2.0v: %s", iface.ocr.v28_29)
            log.info("- 2.9v-3.0v: %s", iface.ocr.v29_30)
            log.info("- 3.0v-3.1v: %s", iface.ocr.v30_31)
            log.info("- 3.1v-3.2v: %s", iface.ocr.v31_32)
            log.info("- 3.2v-3.3v: %s", iface.ocr.v32_33)
            log.info("- 3.3v-3.4v: %s", iface.ocr.v33_34)
            log.info("- 3.4v-3.5v: %s", iface.ocr.v34_35)
            log.info("- 3.5v-3.6v: %s", iface.ocr.v35_36)
            log.info("Card IDentification Register (CID):")
            log.info("Raw Value: %x", iface.card_cid.raw)
            log.info("- mid: %s", iface.card_cid.mid)
            log.info("- oem: %s", iface.card_cid.oem)
            log.info("- prv: %s", iface.card_cid.prv)
            log.info("- pnm: %s", iface.card_cid.pnm)
            log.info("- psn: %s", iface.card_cid.psn)
            log.info("- mdt: %s", iface.card_cid.mdt)
            log.info("RCA: %x", iface.rca)
            log.info("SD CARD Configuration Register (SCR):")
            log.info("Raw Value: %x", iface.card_scr.raw)
            log.info("- SD Spec: %s", iface.card_scr.physical_spec_version())
            log.info("- data_status_after_erases: %s", iface.card_scr.data_stat_after_erase)
            log.info("- SD Security: %s", iface.card_scr.sd_security.description())
            log.info("- SD Bus Widths:")
            log.info("- - 1 bit: %s", (iface.card_scr.sd_bus_widths & 1) != 0)
            log.info("- - 4 bit: %s", ((iface.card_scr.sd_bus_widths >>2) & 1) != 0)
            log.info("- Extended Security Supported: %s", iface.card_scr.ex_security != 0)
            log.info("- Command Support:")
            log.info("- - Speed Class Control: %s", (iface.card_scr.cmd_support & 1) != 0)
            log.info("- - Set Block Count: %s", ((iface.card_scr.cmd_support >> 1) & 1) != 0)
            log.info("- - Extension Register Single Block: %s", ((iface.card_scr.cmd_support >> 2) & 1) != 0)
            log.info("- - Extension Register Multi-Block: %s", ((iface.card_scr.cmd_support >> 3) & 1) != 0)
            log.info("Card-Specific Data Register (CSD):")
            #TODO: break down first few fields more
            csd = iface.card_csd
            log.info("- CSD Structure version: %s", csd.csd_structure)
            log.info("- TAAC: %s", csd.taac)
            log.info("- NSAC: %s", csd.nsac)
            log.info("- Transfer Speed: %s", csd.tran_speed)
            log.info("- CCC: %s", csd.ccc)
            log.info("- Max Read Data Block Length: %s", csd.read_bl_len)
            log.info("- Partial Blocks for Read Allowed: %s", csd.read_bl_partial)
            log.info("- Write Block Misalignment: %s", csd.write_blk_misalign)
            log.info("- Read Block Misalignment: %s", csd.read_blk_misalign)
            log.info("- DSR implemented: %s", csd.dsr_imp)
            log.info("- Capacity: %s", csd.get_capacity())
            if csd.csd_structure == 0:
                log.info("- Max Read Current @VDD min: %s", csd.vdd_r_curr_min)
                log.info("- Max Read Current @VDD max: %s", csd.vdd_r_curr_max)
                log.info("- Max Write Current @VDD min: %s", csd.vdd_w_curr_min)
                log.info("- Max Write Current @VDD max: %s", csd.vdd_w_curr_max)
            log.info("- Erase Single Block Enable: %s", csd.erase_blk_en)
            log.info("- Erase Sector Size: %s", csd.sector_size)
            log.info("- Write Protect Group Size: %s", csd.wp_grp_size)
            log.info("- Write Protect Group Enable: %s", csd.wp_grp_enable)
            log.info("- Write Speed Factor: %s", csd.r2w_factor)
            log.info("- Max Write Data Block Length: %s", csd.write_bl_len)
            log.info("- Partial Blocks for Write Allowed: %s", csd.write_bl_partial)
            log.info("- File Format Group: %s", csd.file_format_grp)
            log.info("- Copy Flag: %s", csd.copy)
            log.info("- Permanent Write Protection: %s", csd.perm_write_protect)
            log.info("- Temporary Write Protection: %s", csd.tmp_write_protect)
            log.info("- File Format: %s", csd.file_format)