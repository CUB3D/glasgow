    ...
    async def on_message(self, m: BaseMsg, iface: MmcInterface):
        if m.cmd == SDCommand.CMD7.value:
            ...
            if not msg.base.crc_valid():
                raise MmcError("CMD7 response has incorrect CRC value")
            if not msg.status.current_state == CardState.Stby:
                raise MmcError("CMD7 response indicated incorrect state")
            if msg.status.error:
                raise MmcError("CMD7 response indicated error")
            return True
